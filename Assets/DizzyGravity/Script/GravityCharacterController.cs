...
public class GravityCharacterController : MonoBehaviour
{
	...
	public void Move(Vector3 move, bool crouch, bool jump)
	{
		...
		m_ForwardAmount = move.z; // Déplacement avant/arrière
		m_TurnAmount = 0;         // Pas de rotation pour le déplacement latéral

		// Déplacement latéral (sans rotation)
		Vector3 lateralMovement = move.x * transform.right;

		// Rotation avec la souris
		float mouseX = Input.GetAxis("Mouse X"); // Récupère l'entrée de la souris
		if (Mathf.Abs(mouseX) > 0.1f)            // Évite les petites oscillations
		{
			transform.Rotate(0, mouseX * m_StationaryTurnSpeed * Time.deltaTime, 0); // Applique une rotation
		}

		// Appliquer les mouvements
		Vector3 totalMove = lateralMovement + move.z * transform.forward;
		m_Rigidbody.velocity = new Vector3(totalMove.x, m_Rigidbody.velocity.y, totalMove.z);

		// Gère le mouvement selon l'état (sol/air)
		if (m_IsGrounded)
		{
		...
		{
			HandleAirborneMovement(0); // Pas de rotation latérale en l'air
		}

		ScaleCapsuleForCrouching(crouch);
		PreventStandingInLowHeadroom();

		// Mise à jour des paramètres pour l'Animator
		UpdateAnimator(move);
	}
